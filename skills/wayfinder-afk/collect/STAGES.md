...
Present each group as its own approve/decline (grouping rules and the card format in [PLAYBOOK.md](PLAYBOOK.md) and [TEMPLATES.md](TEMPLATES.md)). Offer *split this group* as a third option — a group is the agent's guess at coherence, and they may not agree with it. **One group per turn, and the picker stands alone — in every client.** The picker renders only its own fields. On the phone it is a modal over the transcript; in the terminal the card has scrolled away — or was never rendered, because text emitted between tool calls is not reliably shown anywhere. So no context may live outside the picker: the question text itself carries the premise and every call, numbered, one clause each **on its own line** — lines, not prose — with the group's confidence and the queue position in the header chip (`G3/9 · HIGH`) and the confidence again in caps on the first line; the option descriptions carry the stakes. Both are written for a reader who has seen **nothing else** — every name the run coined (a ruling's nickname, a scope, an option letter) is glossed in place or left out, because "the latch ruling stays scoped" means nothing to someone who never saw the card. The card still prints, for the full why and reversal costs — but it is a supplement, never the load-bearing copy. A group whose calls can't each be said in one clause inside the question is too big to be one group. Never put two groups in one `AskUserQuestion`: tabs render every question line and none of the context. Keep issuing turns until the queue is empty — the last group gets a picker like the first.
...
Ask **one question per turn**, in the card format, with the run's recommendation visible and progress in the picker's chip (`17 of 58`, recomputed as pruning moves the pool — never `Q17`, which is a frontier key). Batch only if they ask for it.
...
3. **Every choice goes through the picker — including the last one — and every picker stands alone.** Groups, questions, probe modes: each is its own `AskUserQuestion`, one per turn until the queue is empty, with the whole decision inside the picker's own fields — no client reliably shows anything else, phone or terminal, and context that lives only in the transcript was never shown. Nothing the human is meant to decide is left as a paragraph they have to answer by hand. Unclicked is unanswered, and a tail of calls applied because nobody objected is not a ratification.
