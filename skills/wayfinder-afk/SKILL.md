...
9. **Closing is the job; filing is the exception.** Before any new ticket, try in order: answer it in the ticket you already hold, fold it into an open frontier as a numbered question, write it as fog. File only what a stranger could take cold and that genuinely blocks the map. Both modes count closed / filed / **net** and report it — a session that leaves the map bigger owes the lead that number in its first line. The asymmetry is the reason: a run can only close `research` and `task`, so every `grilling` or `prototype` ticket it files is one only the human can retire.
10. **Refer to a ticket by its name.** In everything the human reads, a ticket is its title, never a bare `#295` — a wall of ids is illegible. The id rides inside the name's link; it never stands in for it. Numbers the human answers *with* — `D1`, `Q3` — are the exception: those are keys, quoted exactly as the frontier posted them.
