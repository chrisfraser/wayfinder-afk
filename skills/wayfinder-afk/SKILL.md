...
5. **Commit to `wayfinder/*` branches; never touch `main`.** No merging into `main` or any shared branch — merging the run's own `wayfinder/<map>-*` branches together (bench or landing assembly) is fine. Subagents never push, but they **commit incrementally** — branch before the first file, a commit per unit of work — because an API failure kills an agent between tool calls and only commits survive it. The **lead pushes every `wayfinder/*` branch at handover**. They are namespaced and disposable, but unpushed they die with the workspace — taking the bench kit and prototypes the frontiers point at. Restore the branch the user started on when the session ends — **and leave its tree clean**: every file the session made is committed and pushed — folded into the map's one **landing MR** if it's meant to land (draft until the human has reviewed the full diff; the session never un-drafts it and never merges it), on a bare throwaway `wayfinder/*` branch if it's bench or prototype — or deleted. A file `git status` can still name at handover is unfinished work, not residue.
6. **Build for the hardware, never drive it.** No installing, flashing, factory-resetting, and above all **no rebooting** — a reboot has already ended Wi-Fi ADB permanently on a radio and stranded it mid-run. Read-only checks on a device the session explicitly owns — and holds the lease on (invariant 12) — are the only exception, and in collect the human can lift the ban in the turn they lift it.
7. **Assert on a positive signal, never on the absence of a negative one.** `BUILD SUCCESSFUL`, an exit code you actually captured, a parsed result file — never "grep found no errors". If your shell filters command output, bypass the filter for anything whose *failure* you must see (`rtk proxy <cmd>`, if that's the filter). And **a check whose negative result is ambiguous is not a check**: a "found nothing" that can't distinguish *the thing is missing* from *my pattern, pathspec, or escaping is wrong* proves nothing — and its "found it" can be a coincidence another writer produced. Before trusting either answer, run a positive control (does the check fire where the thing is known to exist?) and a discrimination control (does it stay silent where the thing is known absent?). A failed control is **PRECONDITION** — a third outcome, never counted as either verdict.
...
12. **Assume you are not alone.** Another AFK session may be live on another map right now — same tracker, same repo, same bench. The tracker is safe by claiming (invariant 4) and branches by namespace (invariant 5); the shared checkout and the hardware are not. Do file work in worktrees and leave the checkout's HEAD where you found it. A device is touched only under this map's **lease**: `scripts/device-lease.sh claim <serial> <map>` before the first command reaches it, `release` at session end. A refused lease names the map that has the device — busy is an answer to report, never a lock to wait on or break.
13. **The map ends — steer by `## Done when`.** Every session reads the map's arrival condition first. If the map has none, infer one from the destination and Notes and post it for ratification; if it can't be inferred with high confidence, demand it from the human as the first thing blocked on them — and say the session is steering without one. Every handover scores it — met / **decided but unbuilt** / undecided — beside the net. Decided-but-unbuilt is the cue to ask for the build; all-met is the cue to propose closing the map. A session that cannot say how far away the destination is has measured nothing.
...
scripts/            map-frontier.sh · answers.sh · bench-scaffold.sh · device-lease.sh · landing-verify.sh
```
