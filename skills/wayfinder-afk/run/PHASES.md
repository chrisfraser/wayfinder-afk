...
**Push every `wayfinder/*` branch the run created** (`git push -u origin <branch>` — never `main`, never a merge into anything shared), then restore the user's starting branch. Unpushed, the bench kit and prototypes die with this workspace, and the frontiers' `Settled by:` lines point at files that no longer exist anywhere. With that done, everything durable is server-side — frontiers, review queue, report, branches — and a lost session costs nothing: `bash scripts/map-frontier.sh <map>` rebuilds the picture in one command. When the answers come back, `/wayfinder-afk collect <map>` applies them; **every comment this run posts carries the `<!-- wayfinder:agent -->` stamp** so collect can tell them from the human's replies.
