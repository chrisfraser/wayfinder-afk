...
**Leave nothing uncommitted — walk `git status --porcelain` to empty.** Every line it prints (filter bypassed, so a line can't be swallowed) is unfinished business, and each one gets a disposition said out loud: committed to the branch of the ticket it belongs to, committed to a throwaway `wayfinder/*` branch, or deleted. Then give each surviving branch its ending: work meant to land in the product gets a **draft MR** (`glab mr create --draft`, crib in PLAYBOOK.md) so the human reviews a diff instead of excavating a branch; bench kits and prototypes are throwaway — pushed is their terminal state, no MR. The report names which branch got which. **Release every hardware lease the run holds** (`scripts/device-lease.sh release <serial> <map>`) — an unreleased lease blocks another map's run for the rest of its TTL. With that done, everything durable is server-side — frontiers, review queue, report, branches — and a lost session costs nothing: `bash scripts/map-frontier.sh <map>` rebuilds the picture in one command. When the answers come back, `/wayfinder-afk collect <map>` applies them; **every comment this run posts carries the `<!-- wayfinder:agent -->` stamp** so collect can tell them from the human's replies.
