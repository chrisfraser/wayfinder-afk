...
- Landing MR — **lead only, at handover; one per map, never one per branch**. Assemble `wayfinder/<map>-landing` in a worktree — created off `origin/main` on first use, reused after — merging each landable ticket branch in ascending iid order; a non-trivial conflict keeps that branch out and goes in the review queue. Push, then check for the MR before creating one: `glab mr list --source-branch wayfinder/<map>-landing` — an open MR just grew; none → `glab mr create --draft --source-branch wayfinder/<map>-landing --target-branch main --title "wayfinder #<map>: <destination>" --description "<what rode in, ticket by ticket>"`. **Draft is the review gate**: the human reviews the full diff before it lands, and the run neither un-drafts nor merges — ever. Bench and prototype branches get no MR; pushed is their terminal state.
- Landing verification — **lead only, between merge and push**: `bash scripts/landing-verify.sh check origin/main wayfinder/<map>-landing <ticket branches…>`. Non-evidence for content arrival, each learned at cost: a merge that exited 0; the absence of conflicts; `git log -- <path>` (follows the stale parent, blind to this class — `--full-history` is the minimum); ancestry scans ("is the branch merged?" — a file a later ticket rewrote reads clean); comparing `<merge>` against `<merge>^2` (reported 275 lost files where 4 were lost — compare against the merge-base). The three-armed marker check is the positive signal: ABSENT keeps that branch out like a conflict; PRECONDITION means fix the *marker*, and counts as neither verdict. This layer is git-specific by design — a project on another VCS drops this bullet and the script, and keeps the rest of Phase 4.
